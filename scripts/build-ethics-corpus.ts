...
import { LOGIC_FOL_V1 } from '../src/data/logic-fol-v1.ts';
import { LATIN_MAP } from '../src/data/latin-map.ts';
import type {
  Dependency,
  EthicsCorpus,
  EthicsItem,
...
type ParsedItem = {
  ...
  number: number;
  ofProposition?: number;
  subIndex?: number;
  textParts: string[];
  translation?: string;
};

type ParsedEnglishItem = ParsedItem & { translation: string };
type LatinItemKind = ParsedItem['kind'];
...
function applyLatinTexts(corpus: EthicsCorpus): void {
  for (const item of corpus) {
    const latin = LATIN_MAP[item.id];
    if (!latin) continue;
    (item as EthicsItem & { latin?: string }).latin = latin;
  }
}
...
function applyFOLv1Logic(corpus: EthicsCorpus): void {
  for (const item of corpus) {
    ...
    const targetLogic = Array.isArray(item.logic) ? item.logic : [];
    item.logic = targetLogic.filter((enc) => !(enc.system === 'FOL' && enc.version === 'v1'));
    item.logic.push(...encodings);
  }
}
...
function validateCorpus(corpus: EthicsCorpus): void {
  ...
  if (e1d1) {
    ...
    if (folV1.length === 0) {
      console.warn('[Logic WARN] E1D1 is missing FOL v1 encodings.');
    }
  }
